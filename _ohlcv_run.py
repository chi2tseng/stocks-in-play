# -*- coding: utf-8 -*-
# Phase 10b OHLCV fetch. Usage: py _ohlcv_run.py <TARGET_ISO> <TODAY_ISO>
import json, os, io, csv, sys, urllib.request
...
for s in studies:
    ...
    if row.get('prev_close'):
        s['snapshot']['chgPct'] = round((row['close'] - row['prev_close']) / row['prev_close'] * 100, 2)
    changed += 1
if changed:
    with io.open(sp, 'w', encoding='utf-8') as f:
        json.dump(studies, f, ensure_ascii=False, indent=2)
print(f'[OK] studies backfilled: {changed}')
