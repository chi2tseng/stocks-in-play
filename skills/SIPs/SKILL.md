...
**交易機會分頁(2026-08-06 使用者需求):** 每日 /SIPs 由主模型寫 `trade_ideas.json`(rank/symbol/setup/direction/thesis/entry/target/stop/backtest_note),build 注入 `DATA.tradeIdeas` 渲染於「交易機會」分頁。setup 引用已回測的樣態(DTC>5 軋空、大合約 fade、好財報錯殺反彈等);未回測的 setup 必須在 backtest_note 標「樣本累積中」並降倉位建議。
