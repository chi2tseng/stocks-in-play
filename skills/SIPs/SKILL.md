...
|---|---|---|---|---|
| **0. 模型對帳(每日必跑,開跑第一步)** | `py model_lab.py verify` + `py model_lab.py ingest`(驗最近已收盤日 + 收樣本進事件庫;印 `[!! MODEL-DRIFT !!]` 就接著跑 `py model_lab.py refit`);**0c Daily Case 對答案**:`py daily_case.py resolve <昨日>` → 上網查昨日那檔**真正為何**漲/跌(開盤後的走勢原因,不是盤前的) → `py daily_case.py cause <昨日> "<原因>" "<教訓>"` | ~1-2 min | $0 | verify 一行結論進 brief;`model_events.json` / `model_track_record.json` 更新;refit 後同步 `trade_setups.json` 的 model 欄 |
...
| **8b. Write sean_analysis.json** | **1 sonnet agent** 照 `docs/SEAN_STYLE.md` 幫每檔 claude_picks 寫「Sean 視角」判讀(§ 8.1b;**2026-09-16 起精簡版:verdict + 一句 summary,併入新聞詳情卡底部,不再獨立卡片**) | ~2 min(背景) | $0 | `sean_analysis.json` |
| **8c. Write milan_analysis.json** | **1 sonnet agent** 照 `docs/MILAN_STYLE.md` 幫每檔 claude_picks 做「Milan 視角」催化劑 0-10 評級(§ 8.1c;**精簡版:score + 一句 summary,併入新聞詳情卡底部**;與 8b 平行發) | ~2 min(背景) | $0 | `milan_analysis.json` |
...
| **9d. 交易機會(Ideas,硬性盤前完成)** | **主模型**按順位 v3 選**至多 2 檔**寫 `trade_ideas.json`:gap 一律用 `prepost_quote.py` 拉真實盤前價(**禁用 packet chgPct** — range=1d 陷阱),TV 數據缺就先 `tv-scrape.js` 補;無合格標的照樣寫檔(空 ideas + `no_trade_notes` 說明各 setup 為何不合格)。**必須在 21:30 台灣(9:30 ET 開盤)前寫完** — 開盤後補寫=不算預測(mtime 為準)。若 /SIPs 在 20:00 前跑,此步驟延到 20:00-21:00 盤前 gap 定型後補做再 rebuild+push | ~5 min | $0 | `trade_ideas.json`(rank/symbol/setup/direction/thesis/entry/target/stop/backtest_note/written_at);**9e Daily Case(每日一檔,硬性盤前)**:`py daily_case.py pick` 列流動性候選(市值≥10 億、20 日均成交額≥2 千萬美元、非樂透、|gap|≥3%)→ 挑**一檔**最可能上漲或下跌的 → `py daily_case.py choose <DATE> <SYM> up\|down "<論點>"`(written_at 為 ex-ante 證據;隔日 Step 0c 對答案+歸因,案例庫 `daily_case.json` 顯示在 Ideas 頁頂端)|
...
1. **跑 `py bignames-scan.py`**(在 §2.0 pre-scan 同批發射,~40–60s)—— 掃 ~158 檔大型股宇宙(市值 >$10B),印出當日 `|chg| ≥ 2%` 的名字,MISSING(不在 candidates.csv)與 already-in 分兩區。門檻可調:`py bignames-scan.py 3`。
1a. **頭條欄位:預設就跑,不需要任何旗標(2026-07-20)。** 每一列下面掛一行 `↳ [時間 媒體] 標題` + 一行 URL,取自 `headline.py`(Yahoo Finance 新聞源,36 小時內、優先標題含公司名、扣分排除大盤 wrap/futures 類)。同時寫出 `scan_headlines.json`(sym → chg/last/in_csv/title/url/publisher)供下游取用。
   - **`--no-news` 只用於離線 debug,日常流程禁用。** 掃描輸出沒有頭條欄 = 這一輪掃描不算數,重跑。
   - 印出 `NO HEADLINE FOUND (<36h)` 的名字**不是可以略過**,是**必須手動獵捕催化劑**的旗標(§2.1 fan-out / CNBC-WSJ-Reuters 頭條 / 公司 IR / SEC),查完才決定收不收。
...
2. **bignames-scan 已改為盤前/盤後感知**(includePrePost 5 分 K 最後成交 vs 前收),盤前跳空不再隱形 — 但日曆閘門仍是主網,報價網是輔助。
   - **2026-07-20 基準日 bug 修正**:兩個腳本的 %chg 統一改走 `prepost_quote.py`(`range=2d&interval=5m&includePrePost=true`,依 `meta.gmtoffset` + `currentTradingPeriod.regular.start` 切出**今日**交易所日期的 5 分 K,取最後一筆對比前一交易日官方收盤)。舊版 `range=1d` 在盤前拿到的是**前一交易日**的 K 線、`chartPreviousClose` 又指向再前一日,於是印出「週五 vs 週四」並當成當日盤前(實測 ISRG −13.75%、NFLX −7.72% 全是假的;真值 +0.41% / −0.57%)。
...
5. **SCANX 分桶鐵則(2026-07-21 使用者;同日補充「earnings 和 guidance 要分開兩區」):** SCANX 每個方向分**三桶**:
   ...
   - **Other** = 其餘一律往下放:報前卡位(今晚才報)、**舊財報/舊指引的餘波飄移(例 MU 的 HBM 售罄敘事+分析師調 PT,報告是幾週前的 → false)**、分析師動作、族群連動。
   機制:`build_dashboard.py` 依 `catalysts_today.json` 的 **`"EarningsReaction": true/false`** + `Type`(earnings vs guidance)分桶;無旗標後備 = Type∈{earnings,guidance} 且無報前措辭。**寫 catalysts_today.json 就要誠實標**:報前一律 `false`;>1-2 交易日舊事件餘波 `false`;guidance 桶只給「今天發布的獨立指引事件」。
...
**TV 新鮮度硬規則(2026-08-06 使用者:「確保 TV 數據都是最新季度符合財報」;AMD 8/5 事故 — 沿用財報前快照):** 今日 earnings-reaction 股的 TV **必須當日、財報後重刮**,不准沿用昨日 md。程式檢核:`parse_tv` 會抽 `NextReportDate`(含 Today/Tomorrow 相對日,以刮取時間換算),**nextReportDate ≤ 包日期 ⇒ 那份 TV 是財報前刮的**,build 印 `[!! TV-STALE !!]` 列名單 — 重刮+parse+rebuild 到閘門靜音才准 push。TV 網站本身尚未更新該季(重刮後仍 stale)→ 在 news_detail 標「TV 尚未收錄本季,表列為上一季」並於 late sweep 再試一次。
...
1b. **熱門層 — mcap≥$100B ∪ 當日成交金額 top10 ∪ headline ∪ theme_watch.json(2026-08-06 使用者:「大股票熱門股票都要有詳細的分析」;SNDK/WDC 漏抓事故):≥300 字詳析**,`[!! NEWS-THIN-HOT !!]` 閘門擋 <300 字。**theme_watch.json 維護(§2.0e):** 主題族群領頭羊清單,不受市值 universe 與 ±4% 門檻限制 — bignames-scan 對名單內股票用 **|chg|≥1.5%** 就收錄(Session=headline);每日 §2.0 cluster map 出現的族群領頭羊**補進**名單、連續 10 個交易日沒進過 cluster map 的**移除**。
...
- **⚠ 中文可讀性硬規則(2026-07-31 使用者:「確保寫出來的都是能夠易讀的中文,不要再出這種錯」— AAPL 事故):** 所有繁中輸出(catalyst 一句話、news_detail、Sean/Milan、chat brief)以**台灣財經媒體的白話**為準,**禁止英文術語直譯腔**。已踩過的雷 → 正確寫法:「大中華」裸用 → **中國市場(大中華區)營收**;「街上」(the Street)→ **華爾街預估**;「六月季」(June quarter)→ **6 月底止季度**;「倉位面」(positioning)→ **籌碼面**;「未經調和」(unreconciled)→ 白話寫「數字有出入」;「季對季壓縮」→ **比上季下滑**;「有機成長」(organic growth)→ **本業成長**(必要時括號註明不含併購;2026-08-05 KTOS 事故)。**2026-08-05 全站掃除追加(共修 574 處,重犯最多的先列):**「板塊」→ **類股/族群**(234 處!);「重評」→ **重新評價**;「驚奇度/驚奇」→ **意外程度/超乎預期**;「事實包」(fact pack 內部術語)→ **查證資料**;「分析師定位」→ **分析師評等動態**;「去風險」→ **風險解除**;「賽道」→ **領域**;「自主可控」→ **自主性**;「覆蓋分析師」→ **相關分析師**;句中裸英文(guidance、teens)→ 翻成中文。判準:一個詞若不確定台灣讀者能否**秒懂**,就不要照英文造詞,用一句白話把意思講出來;公司官方分部名可括號附註。**寫完自查一遍**:通篇唸起來像台灣財經記者寫的,才算過。
...
模仿 **Sean Sharpe(Stocks in Play substack)** 的分析方法,幫**每檔 claude_picks** 寫一份獨立分析;**2026-09-16 使用者改版:不再是獨立卡片 — 渲染成新聞詳情卡底部的一行「判讀」**(`Sean · <verdict> · <一句>`)。決策樹照跑,但**只輸出結論**,使用者要的是精簡資訊。
...
- **Schema(2026-09-16 精簡版):`{ "SYM": { "verdict": "MAIN WATCH|WATCH|PASS|AVOID", "summary": "<一句,≤60 字>", "sourceDate": "YYYY-MM-DD" } }`**。summary 是交易員一句話:為什麼是這個等級 + 進場條件或觀望原因。舊欄位 `analysis`(長文)可省;若同時給,渲染只用 verdict/summary。**只准用 packet 既有數據,缺欄寫 N/A,禁止編造數字**。
...
- **Schema(2026-09-16 精簡版):`{ "SYM": { "score": <0-10>, "summary": "<一句,≤60 字:分數的理由>", "sourceDate": "YYYY-MM-DD" } }`** → `milan_analysis.json`。措辭拆解與意外程度測試在腦內跑,只輸出分數與一句理由;`analysis` 長文可省。
- 交給 **1 個 sonnet agent**(與 Sean 的 agent 平行發);允許 WebSearch 查分析師定位與近 60-90 天新聞流(一級源、查詢帶 ISO 日期),數字禁編造、查不到寫查不到。
- **輸出全白話**(同 § 8.1b 規則):只有 score + summary 一句;禁用內部框架術語;不給進出場建議(那是 Sean 那行的事)。**§8.1 的中文可讀性硬規則同樣適用。**
- **分工鐵則:** news_detail 正文仍照 2026-07-06 指示只放純新聞 —— Sean/Milan 判讀只存在自己的 json,由 build 併到卡片底部的判讀列,不得寫進 news_detail.json 正文。
...
**模型預測卡重建(2026-08-07 使用者抓包「預測=gap+5%」— 12 輪盲測審計後定版):** 審計證實 predDay 各 segment gap 斜率 0.9-1.1 ≈ gap 回聲,且裸基線「預測=gap」盲測命中 44.5% > v12 42.9% — **模型對收盤位置無附加資訊,卡片上誠實標示「收盤慣性基準(≈gap)」,不得再宣稱預測收盤**。主打改為**預測盤中最高**,且必須**逐檔**(2026-08-07 二次抓包:分桶常數=同類全同值,又是 gap+固定%):現行 = **spike GBM**(`model_lab.py spikefit` 訓練 sklearn HistGBR 分位模型 + split-conformal 帶 → `model_spike.pkl`;`model_predict` 逐檔推論,分桶表僅 fallback)。盲測成績(R15/R16b):**桶內 IC +0.206**(分桶常數版按定義=0 — 這是逐檔區分的判準)、整體 IC 0.396、逐月 0.35-0.40、conformal 帶內率 45%(理想 50%)。特徵=gap/rot/市值/DTC/short float/float/量/surprise/YoY/perf/type 全因子。**卡片定版(2026-08-07 使用者指示):標題「IDEA」、三行全英文、逐數字紅綠** — `Gap at scan` / `Intraday high range`(spike GBM P25-P75 換算 vs 昨收)/ `Close range`(predDay+band);中位、再衝、慣性基準、v12 副標、到價率註記全部不上卡。質化判讀與 setup 旗標保留。到價率(盲測基準:掛區間下緣 67%、中位 46%、上緣 27%,n=3,410)由 verify 每日追蹤,連兩日大幅偏離 = 失準重跑 spikefit;refit 會一併重訓。**⚠ 改模型程式後必須重啟 sidecar**(長駐進程模組快取,舊碼會把新注入蓋掉 — 2026-08-07 實際發生)。16 輪完整紀錄在 model_iterations.json(含:溫和低估做多、spike-capture 掛單、泛化 over-gap fade 全部陣亡的誠實負結果)。
...
3. **refit 節律:** 每週五或 MODEL-DRIFT 觸發時 `py model_lab.py refit` — 全資料重擬合 + 嚴格 walk-forward 盲測,**盲測變差就不更新** model_params.json(防過擬合單日雜訊)。首戰教訓(8/5 verify):實盤命中 25% vs 回測 42%,主因=盤前快照 vs 開盤 gap 中位差 6.1pt(輸入時點)與 na 層市值缺失 — 修輸入優先於改係數。
**質化調整層(2026-08-06 使用者:「更多質化分析,不要非黑即白」):** 每日 /SIPs 由**主模型**(不得委派)對 picks + 熱門集合 + |gap|≥10 的股票寫 `model_qual.json`:`{SYM:{date, adj(±8 連續值), confidence, reasoning}}` — reasoning 必須回答「為什麼這檔會偏離量化基準」(樣本外情境/預期定位/族群 regime/事件品質),詳細頁卡片會顯示。**質化層自己也被 verify 分開追蹤**(純量化 vs 加質化的 MAE 對比)— 判斷加不加值由數據裁決。**樂透帶不做點估調整(2026-08-07 首日對帳教訓):** 前日 >100% 或無消息連噴的股票(WYHG/CLRO 型)本質不可預測 — 對它們做方向性下修在 MAE 上是純減分(8/6 質化層整體 −1.08 全虧在這);此類只標 confidence=low + reasoning 註明不可預測,adj 一律 0,把調整額度留給真有資訊的檔。
**交易機會分頁(2026-08-06 使用者需求):** 每日 /SIPs 由主模型寫 `trade_ideas.json`(rank/symbol/setup/direction/thesis/entry/target/stop/backtest_note),build 注入 `DATA.tradeIdeas` 渲染於 Today's SIPs 的 **Ideas** subtab(MAGNA53 旁;卡片沿用 sip-card 設計,別另開獨立分頁)。setup 引用已回測的樣態(DTC>5 軋空、大合約 fade、好財報錯殺反彈等);未回測的 setup 必須在 backtest_note 標「樣本累積中」並降倉位建議。
**Ideas 誠實性(2026-08-07 使用者抓包後立,硬規則):** trade_ideas.json 必須**盤前(美東 09:30 前)寫完**;開盤後補寫或改寫=盤中看答案,不算預測。verify 對帳以檔案 mtime 為準,只認寫入之後的走勢;8/6 首日盤中寫的那批不計入戰績。
**錯殺反彈進場規則(2026-08-07 一個月 ex-ante 回測定調,同日 Iter1-5 補強):** 開盤直接進場=負期望(7/7–8/5 avgR −0.22,停損加寬到 6% 也救不回);標準版=「**10:30 ET 前不進場** + **承接濾網**(10:30 價需高於早盤低點 0.5% 以上,貼著低點不進)+ **結構停損**(早盤低點下 0.5%,非固定 2.5%)」— ≥10B 樣本 64% 勝率/+0.48R(n=33)。市值 ≥10B 優先。
**DTC>5 軋空滾動開關(2026-08-07 Iter4):** 開盤進 3R 目標不變,但每天先看**前 10 筆已完成 DTC5 的累積 R**:>0 才做,≤0 跳過或減半(ex-ante 可執行;6-8 月測試全期 +0.13→+0.29R,7 月壞 regime 虧損砍 60%,被擋的平均 −0.20R)。
**全檔案庫定調(2026-08-07 Iter6,5/13–8/5 60m bars;60m vs 5m 粒度已交叉驗證一致):**
- **錯殺反彈(10:30+承接+結構停損,≥10B)= 主力 setup**:全庫四個月皆正,+0.60R/筆(n=48),對照開盤進場 +0.07 — 升級為「全庫驗證」。
- **大合約 fade short = 次主力**:全期 71% 勝 / +0.47R(n=28),兩獨立窗口重現。
...
**每日只出 2 筆(常設;順位 v5 — 2026-08-08 全面重建版,正本=trade_setups.json):** trade_ideas.json 每天**最多 2 檔**,寧缺勿濫。順位:①錯殺反彈 ≥10B(按市值)②大合約 fade(按 gap)③**BeatenSqueezeGo**(gap≥5 + perf1M≤−20 + short float≥10 + DTC<5 + fwd_rev≥30;開盤買/−5% 停/13:30 出;n=45、58% 勝、+0.59R,7 月集群風險註記)④cap10×10 ⑤錯殺反彈 <10B ⑥DTC 假軋空 fade。**多單先過 avoid_rules(重點:無回補買盤家族 — gap>50&DTC<0.5、微型股 gap+10~50 全帶、拋物線崩解、殘股;M&A 現金對價日整檔剔除)**;全部當日了結。每個 setup 帶預註冊 kill criteria 與 live_ledger,verify 觸發即執行。詳細規則/統計/my_take 一律以 `trade_setups.json` 為唯一正本,SKILL 不再重複展開。全庫回測(59 天):**v4 陣容 n=95、勝率 55%、+0.45R/筆、+42.7R、四個月皆正**(DTCfade 槽 12 筆 75% 勝 +0.84R;四個 setup 全正貢獻 — 2026-08-08 R25 驗證)。**全部為 in-sample 調出 — live ex-ante 對帳(8/7 起)才算真驗證。**
