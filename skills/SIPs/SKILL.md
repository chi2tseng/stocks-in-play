...
**每日只出 2 筆(2026-08-07 使用者指示,常設;順位 v4,2026-08-08 更新):** trade_ideas.json 每天**最多 2 檔**,寧缺勿濫。選擇順位:①錯殺反彈 ≥10B(同 setup 內**按市值排序**,不按 surprise;同分 tie-breaker 選 fresh — perf1M ±10% 內)②大合約 fade(按 gap 大小)③cap10×10(市值 ≥10B + 任一成長/驚奇 ≥100% + gap ≥+4;開盤進、停 2.5%、4R 到就走否則 13:30 ET 出,**不抱收盤**)④錯殺反彈 <10B ⑤**DTC 假軋空 fade(空單,2026-08-08 取代 DTC5 多單)**:DTC>5 + gap≥+2 → 10:30 ET 放空、停損 +5%、收盤回補(全庫 n=149、53% 勝、+0.17R、4/4 月正;舊多單全做 ≈0 已除役)。**選股前先過 trade_setups.json 的 avoid_rules 禁區**(gap>50 且 DTC<0.5 的散戶真空 blowoff 禁多;perf12M<−90 且 |gap|>30 的殘股雙向禁區;perf1M>100 且 gap>30 禁隔夜多)。全庫回測(59 天):v3 陣容 n=93、勝率 52%、+0.46R/筆、**+42.6R、四個月皆正**(v2 無 cap10×10 為 +42.7R 但 8 月為負 — v3 用 gap-up 腿換月度平滑)。**全部為 in-sample 調出 — live ex-ante 對帳(8/7 起)才算真驗證。**
