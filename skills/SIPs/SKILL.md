...
**模型預測(2026-08-05 起):** build 結尾自動執行 `model_predict.py`,把 v12 模型預測寫進每檔股票的 `modelPred` 欄位(盤前跳空 → 預測收盤 % + 50% 區間 + 預測盤中最高;`type=contract` 且 gap≥15% 自動掛「大合約 fade」警語)。詳細頁渲染成「模型預測 Model Forecast」卡(header 下方第一張)。**係數在 `model_params.json`,由模型迭代管線(scratchpad 的 all_events/v11 系列)導出 — 勿手改;新資料累積後由迭代 loop 重新導出。** 預測基於掃描時盤前 %(與開盤 gap 有落差,屬已知近似)。
