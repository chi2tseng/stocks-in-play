...
**質化調整層(2026-08-06 使用者:「更多質化分析,不要非黑即白」):** 每日 /SIPs 由**主模型**(不得委派)對 picks + 熱門集合 + |gap|≥10 的股票寫 `model_qual.json`:`{SYM:{date, adj(±8 連續值), confidence, reasoning}}` — reasoning 必須回答「為什麼這檔會偏離量化基準」(樣本外情境/預期定位/族群 regime/事件品質),詳細頁卡片會顯示。**質化層自己也被 verify 分開追蹤**(純量化 vs 加質化的 MAE 對比)— 判斷加不加值由數據裁決。
