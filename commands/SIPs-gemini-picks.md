...
sister_command: SIPs-gemini-full (full pipeline + writes shared state)
related: SIPs-codex-picks (ChatGPT version)
---

# /SIPs-gemini-picks — your own picks, no shared-state writes

You are running as **Gemini CLI (Google)**. The day's scan (Barchart gap scan,
TradingView FQ scrape, Finviz shorts data) was already produced by another agent —
Claude's `/SIPs` or `/SIPs-codex-full`. You **don't re-run the scraping** — those
...
The single hard rule: **your file writes are scoped to `gemini_picks.json` only**. The
...
This is where you earn your spot vs. Claude / ChatGPT. Don't just copy their picks.
...
You're competing with Claude and ChatGPT for the user's attention on which agent
...
ChatGPT's `codex_picks.json`. That defeats the purpose of having three independent
curators — the user is comparing your pattern recognition to theirs.

### 5. Write `D:\SIPs\gemini_picks.json`
...
These belong to Claude / ChatGPT / the user. Picks mode is about doing your OWN analysis
...
- "Wrote M picks to gemini_picks.json: K longs + L shorts"
