...
- **Codex / ChatGPT(codex CLI)** → 你是評審之一。
  **讀 `C:\Users\chi2t\.codex\skills\SIPs\SKILL.md`(統一版,預設從頭跑完整一輪),從頭到尾照做。**
  = (包新→讀;**包舊或不在→重掃 barchart 抓新增**,你有終端機)→ 自己上網查證 → 寫 `codex_picks.json` → build+push。

- **Gemini(agy / Antigravity CLI)** → 你是評審之一。
  **讀 `C:\Users\chi2t\.gemini\skills\SIPs\SKILL.md`(統一版,預設從頭跑完整一輪),從頭到尾照做。**
...
- **Grok(grok CLI)** → 你是評審之一。
  **讀 `C:\Users\chi2t\.grok\skills\SIPs\SKILL.md`(統一版,預設從頭跑完整一輪),從頭到尾照做。**
...
3. 掃描包 `dashboard/data/<今日YYYY-MM-DD>.json` **舊了(scanTimestamp 距今 > ~10 分)或不存在** → **不要停、也別只吃舊包,重掃 barchart 抓新增名字**:
   ...
   Gemini 在 headless/accept-edits 無終端機時:包在就直接用、包缺才 web-discovery 自建薄包。細節見各自統一版 SIPs skill § 1。
