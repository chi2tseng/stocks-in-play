...
function addStudy(stock, opts = {}) {
  ...
  // Default behaviour dedups on (symbol, scanDate) so the SIP-card "Save to Studies" toggle
  // doesn't accidentally make a duplicate when clicked twice for the same scan. The Studies
  // search box (and any other "always-add-a-fresh-row" caller) sets `force: true` to bypass
  // this — that's how duplicate studies for the same ticker on the same date become possible.
  if (!opts.force && arr.some(st => st.symbol === stock.symbol && (st.snapshot?.scanDate || '') === scanDate)) return false;
...
function addManualStudy(rawSym) {
  ...
  // Manual placeholder creates ALWAYS make a new study — duplicates allowed by design
  // since the same ticker may be tracked under different catalysts at different times.
  const arr = loadStudies();
