...
/* ── Screenshot grid + upload zone + thumbs + lightbox ── */
.study-screenshots {
...
.study-shot img { width: 100%; height: 90px; object-fit: cover; display: block; background: var(--surface-elevated); transition: transform 0.15s; }
.study-shot img:hover { transform: scale(1.04); }
.study-shot-bar { display: flex; gap: 4px; padding: 4px 6px; background: var(--canvas); }
.study-shot-label { flex: 1; min-width: 0; padding: 3px 6px; font-size: 11px; border: 1px solid transparent; border-radius: var(--r-sm); background: transparent; color: var(--body); font-family: var(--font-body); }
.study-shot-label:focus { outline: none; border-color: var(--primary); background: var(--canvas); }
.study-shot-del { padding: 2px 6px; font-size: 11px; background: transparent; color: var(--mute); border: none; cursor: pointer; border-radius: var(--r-sm); }
.study-shot-del:hover { color: var(--neg); background: rgba(226,59,74,0.08); }
.study-shot-drop {
  display: flex; align-items: center; justify-content: center;
  min-height: 130px; border: 2px dashed var(--hairline); border-radius: var(--r-sm);
  cursor: pointer; transition: border-color 0.12s, background 0.12s;
  color: var(--mute);
}
.study-shot-drop:hover, .study-shot-drop:focus, .study-shot-drop.drag-over { border-color: var(--primary); background: rgba(73,79,223,0.04); color: var(--primary); outline: none; }
.study-shot-drop-inner { text-align: center; padding: 8px; }
.study-shot-drop-icon { font-size: 28px; font-weight: 300; line-height: 1; margin-bottom: 4px; }
.study-shot-drop-hint { font-size: 10px; line-height: 1.4; }
...
<nav class="topbar" id="nav">
  <div class="topbar-inner">
    ...
    <div class="nav-links">
      ...
      <a data-route="studies" data-i18n="nav-studies">Studies</a>
    </div>
    <div class="topbar-right" id="topbar-right">
      <button class="topbar-iconbtn" id="lang-toggle" aria-label="Toggle language" title="切換語言 / Toggle language">
        <span id="lang-label">EN</span>
      </button>
      <button class="topbar-iconbtn" id="theme-toggle" aria-label="Toggle dark mode" title="切換深色模式 / Toggle dark mode">
...
// ── i18n: bilingual UI dict + language toggle (persisted in localStorage). ──
// Dynamic content (catalysts, news_detail) falls back to ZH if no `_en` field exists;
// going forward /SIPs writes both ZH + EN to claude_picks.json / news_detail.json / catalysts_today.json.
const I18N = {
  zh: {
    'nav-sips': "Today's SIPs",
    'nav-squeeze': 'Short Squeeze',
    'nav-earnings': 'Earnings Results',
    'nav-catalyst': 'Catalyst Deep Dive',
    'nav-scanx': 'SCANX',
    'nav-gappers': 'Gappers',
    'nav-studies': '我的學習',
    'cal-btn-default': '選擇日期',
    'page-todays-sips': "Today's SIPs",
    'page-squeeze': 'Short Squeeze',
    'page-earnings': 'Earnings Results',
    'page-catalyst': 'Catalyst Deep Dive',
    'page-scanx': 'SCANX',
    'page-gappers': 'Gappers',
    'page-studies': '我的學習庫',
    'subtab-claude': 'Claude 精選',
    'subtab-magna': 'MAGNA53 排序',
    'mismatch-hide': '⊘ 隱藏方向不符',
    'mismatch-show': '✓ 包含方向不符',
    'save-study': '➕ 加入學習',
    'saved': '✓ 已加入',
    'remove-study': '移除',
    'studies-empty': '尚未儲存任何 SIPs。在任一張 SIP 卡片上點「加入學習」即可儲存。',
    'studies-note-placeholder': '加入你的觀察筆記（markdown 支援）...',
    'studies-tags-placeholder': '標籤（逗號分隔）',
    'studies-target-price': '目標價',
    'studies-stop-loss': '停損價',
    'studies-conviction': '信念度',
    'studies-saved-on': '加入於',
    'studies-export': '匯出 JSON',
    'studies-import': '匯入 JSON',
    'studies-clear': '清空全部',
    'studies-clear-confirm': '確定清空所有 studies？此動作無法復原。',
    'studies-ohlcv-label': '當日 OHLCV',
    'ohlcv-date': '日期',
    'ohlcv-open': '開盤',
    'ohlcv-high': '最高',
    'ohlcv-low': '最低',
    'ohlcv-close': '收盤',
    'ohlcv-volume': '成交量',
    'studies-screenshots-label': '截圖（進場 / 停損 / 出場 / 圖型）',
    'studies-shot-label-placeholder': '標籤（例: entry / stop / exit）',
    'studies-shot-upload-hint': '點擊上傳 · 拖放圖片 · 或於此區按 Ctrl+V 貼上',
    'claude-rationale-label': 'Claude 分析',
    'forward-yoy': 'Forward YoY',
    'copy': 'Copy',
    'copied': '已複製!',
    'news-detail': '新聞詳情',
    'catalyst-summary': '催化劑摘要',
    'company-news': 'Company News',
    'eps-quarterly': 'EPS Quarterly',
    'rev-quarterly': 'Revenue Quarterly',
    'reported-vs-estimate': 'Reported vs Estimate',
    'no-tv-data': '無 TradingView 季度估計資料',
  },
  en: {
    'nav-sips': "Today's SIPs",
    'nav-squeeze': 'Short Squeeze',
    ...
    'page-scanx': 'SCANX',
    'page-gappers': 'Gappers',
    'page-studies': 'My Studies Library',
    'subtab-claude': 'Claude Picks',
    'subtab-magna': 'MAGNA53 Score',
    'mismatch-hide': '⊘ Hide mismatched',
    'mismatch-show': '✓ Show mismatched',
    'save-study': '➕ Save to Studies',
    'saved': '✓ Saved',
    'remove-study': 'Remove',
    'studies-empty': 'No SIPs saved yet. Click "Save to Studies" on any SIP card to add one.',
    'studies-note-placeholder': 'Add your observation notes (markdown supported)…',
    'studies-tags-placeholder': 'Tags (comma-separated)',
    'studies-target-price': 'Target',
    'studies-stop-loss': 'Stop',
    'studies-conviction': 'Conviction',
    ...
    'studies-ohlcv-label': "Day's OHLCV",
    'ohlcv-date': 'Date',
    'ohlcv-open': 'Open',
    'ohlcv-high': 'High',
    'ohlcv-low': 'Low',
    'ohlcv-close': 'Close',
    'ohlcv-volume': 'Volume',
    'studies-screenshots-label': 'Screenshots (entry / stop / exit / pattern)',
    'studies-shot-label-placeholder': 'Label (e.g. entry / stop / exit)',
    'studies-shot-upload-hint': 'Click to upload · drag & drop · or Ctrl+V here',
    'claude-rationale-label': "Claude's Analysis",
    'forward-yoy': 'Forward YoY',
    'copy': 'Copy',
    'copied': 'Copied!',
    'news-detail': 'News Detail',
    'catalyst-summary': 'Catalyst Summary',
    'company-news': 'Company News',
    'eps-quarterly': 'EPS Quarterly',
    'rev-quarterly': 'Revenue Quarterly',
    'reported-vs-estimate': 'Reported vs Estimate',
    'no-tv-data': 'No TradingView quarterly estimate data',
  },
};
let LANG = (localStorage.getItem('sips-lang') === 'en') ? 'en' : 'zh';
function t(key) { return (I18N[LANG] && I18N[LANG][key]) || (I18N.zh[key] || key); }
// Pick a localized field from a content object: prefers `field_en` when EN active, else `field`, else ''.
function ti(obj, field) {
  if (!obj) return '';
...
function installLangToggle() {
  const apply = () => {
    ...
    // Translate all [data-i18n] nodes (used on the nav links)
    document.querySelectorAll('[data-i18n]').forEach(el => {
      el.textContent = t(el.dataset.i18n);
    });
    // Re-render whatever page is showing so all renderers re-pull translated strings
  ...
  if (btn) btn.onclick = () => {
    LANG = (LANG === 'en') ? 'zh' : 'en';
    localStorage.setItem('sips-lang', LANG);
    apply();
  };
}
...
function renderStudies() {
  ...
  app.innerHTML = `
    <h2 class="page-title">${t('page-studies')}</h2>
    <div class="studies-toolbar">
      ...
      <span class="subtab-hint" style="margin-left:auto">${studies.length} saved · localStorage only</span>
    </div>
    ${studies.length === 0
      ? `<div class="sip-empty">${t('studies-empty')}</div>`
      : `<div class="studies-grid" id="studies-grid">${studies.map(studyCardHtml).join('')}</div>`}
  `;
  // Wire studies card events
  app.querySelectorAll('.study-card').forEach(card => {
    const sym = card.dataset.sym;
    card.querySelector('.study-notes')?.addEventListener('input', e => updateStudy(sym, { notes: e.target.value }));
    card.querySelector('.study-tags')?.addEventListener('input', e => updateStudy(sym, { tags: e.target.value.split(',').map(s => s.trim()).filter(Boolean) }));
    card.querySelector('.study-target')?.addEventListener('input', e => updateStudy(sym, { targetPrice: parseFloat(e.target.value) || null }));
    card.querySelector('.study-stop')?.addEventListener('input', e => updateStudy(sym, { stopLoss: parseFloat(e.target.value) || null }));
    card.querySelector('.study-conviction')?.addEventListener('input', e => updateStudy(sym, { conviction: parseInt(e.target.value, 10) }));
    card.querySelector('.study-remove')?.addEventListener('click', async () => {
      if (confirm(`Remove ${sym} from studies?`)) { await removeStudy(sym); renderStudies(); }
    });
    // OHLCV inputs — write to the nested ohlcv object on every keystroke (no debounce, payload tiny).
    card.querySelectorAll('.study-ohlcv-field input').forEach(inp => {
      inp.addEventListener('input', e => {
        const field = e.target.dataset.field;
        const cur = loadStudies().find(s => s.symbol === sym)?.ohlcv || {};
        const val = field === 'date' ? e.target.value : (parseFloat(e.target.value) || null);
        updateStudy(sym, { ohlcv: { ...cur, [field]: val } });
        // Refresh the intraday chip without a full re-render
        const o = { ...cur, [field]: val };
        if (o.open && o.close) {
          const intra = (o.close - o.open) / o.open * 100;
          const chip = card.querySelector('.study-intraday');
          if (chip) {
            chip.textContent = `Intraday ${(intra >= 0 ? '+' : '') + intra.toFixed(2)}%`;
            chip.className = `study-intraday ${intra >= 0 ? 'pos' : 'neg'}`;
          }
        }
      });
    });
    // Hydrate screenshot <img> elements from IndexedDB (async load)
    card.querySelectorAll('img[data-needs-img]').forEach(async img => {
      const key = img.dataset.needsImg;
      const data = await getImg(key);
      if (data) img.src = data;
    });
    // Click a screenshot → lightbox
    card.querySelectorAll('.study-shot img').forEach(img => {
      img.style.cursor = 'zoom-in';
      img.addEventListener('click', async () => {
        const key = img.dataset.needsImg;
        const data = await getImg(key);
        if (data) {
          const label = card.querySelector(`.study-shot-label[data-img-key="${key}"]`)?.value || '';
          openShotLightbox(data, label);
        }
      });
    });
    // Edit screenshot label
    card.querySelectorAll('.study-shot-label').forEach(inp => {
      inp.addEventListener('input', e => {
        const key = e.target.dataset.imgKey;
        const studies = loadStudies();
        const st = studies.find(s => s.symbol === sym);
        if (!st) return;
        const ss = (st.screenshots || []).find(x => x.imgKey === key);
        if (ss) { ss.label = e.target.value; saveStudies(studies); }
      });
    });
    // Delete screenshot
    card.querySelectorAll('.study-shot-del').forEach(btn => {
      btn.addEventListener('click', async () => {
        const key = btn.dataset.imgKey;
        const studies = loadStudies();
        const st = studies.find(s => s.symbol === sym);
        if (!st) return;
        st.screenshots = (st.screenshots || []).filter(x => x.imgKey !== key);
        saveStudies(studies);
        await delImg(key);
        renderStudies();
      });
    });
    // Upload zone — file picker + drag-drop + paste
    const drop = card.querySelector('.study-shot-drop');
    const fileInp = drop?.querySelector('input[type="file"]');
    const handleFiles = async (files) => {
      ...
      for (const f of files) {
        ...
        try {
          ...
          st.screenshots.push({ imgKey: key, label: '', caption: '' });
    ...
    // Paste handler scoped to the card (focus the drop zone first, or paste anywhere in the card)
    ...
    card.tabIndex = -1;   // makes the card focusable so paste works without clicking the drop input
  });
  // Toolbar
...
function studyCardHtml(st) {
  const s = st.snapshot;
  const chgCls = s.chgPct >= 0 ? 'pos' : 'neg';
  const intentBadge = s.claudeIntent ? `<span class="tag" style="background:${s.claudeIntent === 'short' ? 'rgba(226,59,74,0.10)' : 'rgba(0,168,126,0.10)'};color:${s.claudeIntent === 'short' ? 'var(--neg)' : 'var(--pos)'}">${s.claudeIntent.toUpperCase()}</span>` : '';
  const rationale = (LANG === 'en' && s.claudeRationale_en) ? s.claudeRationale_en : (s.claudeRationale || '');
  const cat = (LANG === 'en' && s.catalyst_en) ? s.catalyst_en : (s.catalyst || '');
  const o = st.ohlcv || {};
  // Compute intraday % chg from open→close so user gets feedback as they type
  const intraday = (o.open && o.close) ? ((o.close - o.open) / o.open * 100) : null;
  const intradayChip = intraday != null
    ? `<span class="study-intraday ${intraday >= 0 ? 'pos' : 'neg'}">Intraday ${(intraday >= 0 ? '+' : '') + intraday.toFixed(2)}%</span>`
    : '';
  const screenshots = st.screenshots || [];
  return `<div class="study-card" data-sym="${st.symbol}">
    <div class="study-card-head">
      <a href="#/stock/${st.symbol}" class="study-sym">${st.symbol}</a>
      <span class="sip-chg ${chgCls}">${fmtPct(s.chgPct)}</span>
      ${intentBadge}
      <span class="study-saved-on">${t('studies-saved-on')}: ${st.savedAt.slice(0, 10)}${s.scanDate && s.scanDate !== st.savedAt.slice(0,10) ? ` · scan ${s.scanDate}` : ''}</span>
      <button class="study-remove" title="${t('remove-study')}">✕</button>
    </div>
    <div class="study-name">${escapeHtml(s.name || '')}</div>
    ${cat ? `<div class="study-catalyst">${escapeHtml(cat)}</div>` : ''}
    ${rationale ? `<div class="study-rationale">${escapeHtml(rationale).replace(/\n/g, '<br>')}</div>` : ''}
    <div class="study-section-label">${t('studies-ohlcv-label')} ${intradayChip}</div>
    ...
    <div class="study-fields">
      <label class="study-field"><span>${t('studies-target-price')}</span><input class="study-target" type="number" step="0.01" value="${st.targetPrice ?? ''}" placeholder="—"></label>
      <label class="study-field"><span>${t('studies-stop-loss')}</span><input class="study-stop" type="number" step="0.01" value="${st.stopLoss ?? ''}" placeholder="—"></label>
      <label class="study-field"><span>${t('studies-conviction')}</span><input class="study-conviction" type="range" min="1" max="5" value="${st.conviction || 3}"></label>
    </div>
    <input class="study-tags" type="text" value="${(st.tags || []).join(', ')}" placeholder="${t('studies-tags-placeholder')}">
    <textarea class="study-notes" rows="4" placeholder="${t('studies-note-placeholder')}">${escapeHtml(st.notes || '')}</textarea>
    <div class="study-section-label">${t('studies-screenshots-label')}</div>
    <div class="study-screenshots" data-sym="${st.symbol}">
      ${screenshots.map(ss => `<div class="study-shot" data-img-key="${ss.imgKey}">
        <img loading="lazy" data-needs-img="${ss.imgKey}" alt="${escapeHtml(ss.label || '')}">
        <div class="study-shot-bar">
          <input class="study-shot-label" data-img-key="${ss.imgKey}" type="text" value="${escapeHtml(ss.label || '')}" placeholder="${t('studies-shot-label-placeholder')}">
          <button class="study-shot-del" data-img-key="${ss.imgKey}" title="${t('remove-study')}">✕</button>
        </div>
      </div>`).join('')}
      <label class="study-shot-drop" tabindex="0">
        <input type="file" accept="image/*" multiple style="display:none">
        <div class="study-shot-drop-inner">
          <div class="study-shot-drop-icon">+</div>
          <div class="study-shot-drop-hint">${t('studies-shot-upload-hint')}</div>
        </div>
      </label>
    </div>
  </div>`;
...
// Lightbox modal for clicking a study screenshot
function openShotLightbox(dataUrl, caption) {
  const overlay = document.createElement('div');
  overlay.className = 'shot-lightbox';
  overlay.innerHTML = `<img src="${dataUrl}"><div class="shot-caption">${escapeHtml(caption || '')}</div>`;
