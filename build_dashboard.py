...
/* Measure popup (TC2000-style) — fixed corner based on drag direction.
   ...
   Either way it's pinned to a fixed corner (does NOT follow the cursor). */
...
function renderCandleChartFull(container, allBars, opts = {}) {
  ...
  const effectivePlotW = plotW - rightInset;   // bars are drawn inside [padL, padL+effectivePlotW]
  ...
  function render() {
    ...
    // 6. Pane separator
    parts.push(`<line class="candle-grid" x1="${padL}" x2="${W - padR}" y1="${(mainBottom + separatorMargin / 2).toFixed(1)}" y2="${(mainBottom + separatorMargin / 2).toFixed(1)}" opacity="0.8"/>`);
