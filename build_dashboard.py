...
/* Preset palette colors for catalyst types — keyed by token in JS */
.cat-type-earnings  { background: rgba(73, 79, 223, 0.10); color: var(--primary); border-color: rgba(73, 79, 223, 0.30); }
.cat-type-momentum  { background: rgba(255, 178, 30, 0.12); color: #b97c00; border-color: rgba(255,178,30,0.40); }
.cat-type-fda       { background: rgba(0, 168, 126, 0.12); color: var(--pos); border-color: rgba(0,168,126,0.30); }
.cat-type-ma        { background: rgba(226, 59, 74, 0.10); color: var(--neg); border-color: rgba(226,59,74,0.30); }
.cat-type-contract  { background: rgba(94, 158, 214, 0.12); color: #2c6fa3; border-color: rgba(94,158,214,0.35); }
.cat-type-technical { background: rgba(160, 100, 220, 0.12); color: #6a3fb0; border-color: rgba(160,100,220,0.35); }
.cat-type-fbo       { background: rgba(255, 105, 100, 0.12); color: #c54848; border-color: rgba(255,105,100,0.35); }
.cat-type-short-squeeze { background: rgba(255, 178, 30, 0.12); color: #a36500; border-color: rgba(255,178,30,0.40); }
body.dark .cat-type-momentum, body.dark .cat-type-short-squeeze { color: #ffce6a; }
...
.chg-cell.pos .gap-pct, .chg-cell.pos .move-pct { color: var(--pos); }
.chg-cell.neg .gap-pct, .chg-cell.neg .move-pct { color: var(--neg); }
.vol-readout { font-family: var(--font-mono); font-size: 12px; color: var(--mute); margin-top: 2px; }

/* Direction pill (LONG / SHORT) before Gain/Stop — same baseline geometry as other .stat-tag
   pills (4px/10px padding, 11px font, surface-soft bg). Just adds direction-specific tint. */
.trade-pill.direction-pill { font-weight: 700; letter-spacing: 0.5px; font-size: 11px; padding: 4px 10px; }
...
/* Gain / Stop pills inherit .stat-tag look; pos/neg tints are already supplied by their classes. */
.trade-pill { font-size: 11px; padding: 4px 10px; font-weight: 600; }
...
.sip-card .chg-cell.pos .gap-pct, .sip-card .chg-cell.pos .move-pct { color: var(--pos); }
.sip-card .chg-cell.neg .gap-pct, .sip-card .chg-cell.neg .move-pct { color: var(--neg); }
...
async function renderStudyDetail(idOrSym) {
  ...
  // Gain (漲幅) = peak favourable excursion from open · Stop (止損) = worst drawdown from open.
  // The direction depends on the trade intent:
  //   LONG  — you bought at open. Gain = how far it ran UP   (Open → High).
  //                              Stop = how far it dropped   (Open → Low).
  //   SHORT — you shorted at open. Gain = how far it dropped (Open → Low,  i.e. profit).
  //                                Stop = how far it ran UP  (Open → High, i.e. loss).
  // So for SHORT the columns swap: gain is anchored on Low and stop on High. The previous
  // "* dir" sign-flip kept the column anchors wrong, producing gain on High for short trades
  // (which is your loss, not profit). Below picks the right anchor per intent.
  function computeMetrics() {
    const cur = loadStudies().find(st => st.id === id)?.ohlcv || {};
    if (!cur.open) return { gain: null, stop: null };
    let gain = null, stop = null;
    if (intent === 'short') {
      if (cur.low  != null) gain = (cur.open - cur.low)  / cur.open * 100;   // → positive when profitable
      if (cur.high != null) stop = (cur.open - cur.high) / cur.open * 100;   // → negative (max drawdown)
    } else {
      if (cur.high != null) gain = (cur.high - cur.open) / cur.open * 100;   // → positive
      if (cur.low  != null) stop = (cur.low  - cur.open) / cur.open * 100;   // → negative
    }
    return { gain, stop };
  }
