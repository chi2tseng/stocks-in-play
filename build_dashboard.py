...
function loadStudies() {
  try {
    ...
    let mutated = false;
    arr.forEach(st => { if (st && !st.id) { st.id = st.symbol; mutated = true; } });
  ...
  app.innerHTML = `
    <div class="breadcrumb study-detail-breadcrumb">
      <span><a href="#/studies">← Studies</a> &nbsp;»&nbsp; <b>${sym}</b>${(() => {
        ...
        const title = `Click to change the trading-day date. Auto-snaps the focus quarter; OHLCV bars stay until /update-studies refreshes them.`;
        return sd ? `&nbsp;·&nbsp;<input type="date" class="study-detail-date-input" id="study-detail-date-input" value="${sd}" title="${title}">` : '';
      })()}</span>
