...
async function renderStock(sym) {
  ...
  if (!s) {
    ...
    app.innerHTML = `
      <div class="breadcrumb"><a href="#/earnings">Earnings</a> · <a href="#/catalyst">Catalyst</a> · <a href="#/scanx">SCANX</a> &nbsp;»&nbsp; <b>${escapeHtml(sym)}</b>${dateBadge}</div>
  ...
  app.innerHTML = `
    <div class="breadcrumb study-detail-breadcrumb">
      <span><a href="#/studies">← Studies</a> &nbsp;»&nbsp; <b>${sym}</b>${(() => {
        ...
        const title = od
          ? `Trading day from OHLCV. Saved on ${(study.savedAt || '').slice(0, 10)}.`
          : `Saved on this date. Fill OHLCV.date in the Gain/Stop popup to switch to the trading day.`;
        return sd ? `<span class="study-detail-date" title="${title}">&nbsp;·&nbsp;${sd}</span>` : '';
      })()}</span>
