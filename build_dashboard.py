...
async function renderStudyDetail(idOrSym) {
  ...
  app.innerHTML = `
    <div class="breadcrumb study-detail-breadcrumb">
      <span><a href="#/studies">← Studies</a> &nbsp;»&nbsp; <b>${sym}</b>${(() => {
        ...
        const chip = (dates.length >= 1) ? `
          ...
          <div class="research-dates-popup" id="research-dates-popup" role="menu" aria-hidden="true">
            ${dates.map(d => {
        ...
        ` : '';
        return sd ? `&nbsp;·&nbsp;<input type="date" class="study-detail-date-input" id="study-detail-date-input" value="${sd}" title="${title}">${chip}` : '';
      })()}</span>
