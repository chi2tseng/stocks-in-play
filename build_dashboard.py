...
async function renderStudyDetail(idOrSym) {
  ...
  const tradePills = `
    ...
    <button class="tag stat-tag trade-pill ${gainCls}" type="button" id="study-gain-pill" title="${intent === 'short' ? 'Gain = (Low − Open) / Open · raw signed drop · negative = short profit · click to edit OHLCV' : 'Gain = (High − Open) / Open · raw signed rise · positive = long profit · click to edit OHLCV'}">Gain <span id="study-gain-val">${fmtMetric(initGain)}</span></button>
