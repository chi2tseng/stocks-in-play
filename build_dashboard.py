...
function addStudy(stock, opts = {}) {
  ...
  // Dedup on (symbol, scanDate) — same scan's data shouldn't yield two studies, but the
  // SAME ticker captured on DIFFERENT scan dates SHOULD coexist as separate studies (each
  // has its own snapshot.tv.chart, catalyst, day chg, etc.).
  if (arr.some(st => st.symbol === stock.symbol && (st.snapshot?.scanDate || '') === scanDate)) return false;
...
function renderStudies() {
  ...
  async function pickSearchResult(sym, scanDate) {
    ...
    addStudy({ ...snap, symbol: sym }, { scanDate });
    // Navigate to the just-created study (loadStudies order is push-append, so it's last).
    const justAdded = loadStudies().slice().reverse().find(s => s.symbol === sym && (s.snapshot?.scanDate || '') === scanDate);
    location.hash = '#/study/' + (justAdded?.id || sym);
  }
  function createSearchResult(sym) {
    const newId = addManualStudy(sym);
    if (newId) location.hash = '#/study/' + newId;
  }
...
function studyPreviewCardHtml(st, idx) {
  ...
  // + 4 forward estimates (max 5 quarters total). Hidden entirely if the study has no chart
  // data (placeholder studies, or tickers without TV quarterly coverage). Editing only
  // happens on the detail page so accidental clicks here don't mutate values.
  const chart = st.customChart || (s.tv && s.tv.chart) || null;
  const msHtml = chart ? renderCompactReadonlyMsTable(chart) : '';
