...
function sipCardHtml(s, idx) {
  ...
  return `<a class="sip-card ${isFeatured ? 'featured' : ''}" href="#/stock/${s.symbol}" style="text-decoration:none;color:inherit;display:block">
...
function claudePickCardHtml(s, idx) {
  ...
  return `<a class="sip-card claude-pick ${isFeatured ? 'featured' : ''}" href="#/stock/${s.symbol}" style="text-decoration:none;color:inherit;display:block">
    ...
    <div class="sip-meta">${sessTags} <span class="tag ${typeTagClass(s.type)}">${s.type}</span></div>
    ${shortPerfLine(s)}
    <div class="claude-rationale-label">Claude 分析</div>
    ${rationaleHtml}
    ${forwardYoyBlock(s)}
  </a>`;
}
...
async function renderSips(subtab) {
  // Subtabs: 'magna' (default) — MAGNA53 scoring | 'claude' — Claude-curated picks with rationale.
  const tab = (subtab === 'claude') ? 'claude' : 'magna';
  ...
  // Wire subtab clicks → re-route via hash so URL is shareable
  document.querySelectorAll('#sips-subtabs .subtab').forEach(t => {
    t.onclick = () => {
      const sub = t.dataset.sub;
      const isLatest = STATE.date === STATE.dates[0]?.date;
      location.hash = '#/' + (isLatest ? '' : STATE.date + '/') + 'sips' + (sub === 'magna' ? '' : '/' + sub);
    };
  });
  ...
  if (tab === 'magna') {
    ...
    // Claude picks come from DATA.claudePicks (an array of {symbol, rank, rationale}).
    // If empty, render an empty-state with instructions.
    ...
    const enriched = picks
      .map(p => bySym[p.symbol] ? { ...bySym[p.symbol], _claudeRationale: p.rationale } : null)
      .filter(Boolean);
    if (enriched.length === 0) {
      stack.innerHTML = `<div class="sip-empty">尚無 Claude 精選清單。<br><br>在 <code>D:\\SIPs\\claude_picks.json</code> 加入 <code>{"picks":[{"symbol":"X","rank":1,"rationale":"..."}]}</code> 後重 build dashboard 即可看到。</div>`;
      return;
    }
...
function renderGappers() {
  ...
  const baseCols = [
    ...
    { label: 'Price',  sortVal: r => r.last,   render: r => `<td class="num">${fmtPrice(r.last)}</td>` },
    { label: '%Chg',   sortVal: r => r.chgPct, render: r => `<td class="num ${cls(r.chgPct)}">${fmtPct(r.chgPct)}</td>` },
    { label: '|%Chg|', sortVal: r => r.absChg, render: r => `<td class="num ${cls(r.chgPct)}">${r.absChg.toFixed(2)}%</td>` },
    { label: 'Volume', sortVal: r => r.volume, render: r => `<td class="num">${fmtVol(r.volume)}</td>` },
