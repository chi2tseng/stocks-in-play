...
function addStudy(stock, opts = {}) {
  ...
  // Dedupe by symbol — same ticker = same study (research accumulates across days).
  // Previously the dedupe key was (symbol + scanDate), which let duplicates accumulate
  // for the same ticker across days; the user couldn't tell whether a ticker had
  // already been researched. Fixed 2026-05-18 — now we ALWAYS dedupe by symbol and
  // merge each new scan-day's snapshot into existing.datedSnapshots[scanDate].
  // The `opts.force` flag is kept for backwards compat but is now a no-op.
  const existing = arr.find(st => st.symbol === stock.symbol);
  if (existing) {
    if (scanDate) {
      existing.datedSnapshots = existing.datedSnapshots || {};
      if (!existing.datedSnapshots[scanDate]) {
        existing.datedSnapshots[scanDate] = {
          snapshot: { ...snapshot },
          ohlcv:    { ...ohlcv },
          customTypes: [],
          hiddenSections: [],
  ...
  // No existing study — create a fresh one.
...
function addManualStudy(rawSym) {
  ...
  // Dedupe by symbol — if a study for this ticker already exists, return its id so
  // the caller navigates to the existing study (preserves prior research: news,
  // notes, dated snapshots, screenshots, OHLCV history). Previously this function
  // unconditionally created new studies, which led to duplicate symbols and made
  // users unable to tell if they'd already researched a ticker. Fixed 2026-05-18.
  const arr = loadStudies();
  const existingForSym = arr.find(st => st.symbol === sym);
  if (existingForSym) return existingForSym.id;
  ...
  arr.push({
    ...
    // build_dashboard.py / /update-studies un-hide them once real TV data lands.
    hiddenSections: ['eps_chart', 'rev_chart', 'ms_table', 'yoy_block'],
    // Default catalyst tag = "earnings" since the vast majority of manually-added
    // placeholder studies are tracking an earnings event. /update-studies uses this
    // tag to decide whether to run the TV scrape on next refresh. User can change
    // the tag from the Catalyst pill if it's actually not an earnings catalyst.
