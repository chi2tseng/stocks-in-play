...
function renderStudies() {
  ...
  app.querySelectorAll('.study-card').forEach(card => {
    ...
    card.querySelector('.study-remove')?.addEventListener('click', () => {
      if (confirm(`Remove ${sym} from studies?`)) { removeStudy(sym); renderStudies(); }
    });
  });
  // Toolbar
  document.getElementById('studies-export')?.addEventListener('click', () => {
    const blob = new Blob([JSON.stringify(loadStudies(), null, 2)], { type: 'application/json' });
  ...
  document.getElementById('studies-import-file')?.addEventListener('change', e => {
    ...
    reader.onload = () => {
      try {
        ...
        if (Array.isArray(arr)) { saveStudies(arr); renderStudies(); }
