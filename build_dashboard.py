  ...
  if (tab === 'magna') {
    ...
    if (enriched.length === 0) {
      ...
      const cmdHint = tab === 'claude'
        ...
        : `在 ${({codex:'Codex',gemini:'Gemini',grok:'Grok'})[tab]} CLI 跑 <code>/SIPs</code>(統一版,預設從頭跑完整一輪)。`;
  ...
  if (s.modelPred && s.modelPred.predDay !== undefined && s.modelPred.predDay !== null) {
    ...
    modelPredHtml = `<div class="stock-card news-detail model-pred"><h3><span class="material-symbols-outlined">query_stats</span>IDEA</h3>
      <div class="model-pred-grid">
        ...
        ${mp.lottery ? `<div><span class="mp-label">Intraday high range</span><span class="mp-val" style="color:var(--mute)">unpredictable · nano-cap, premarket quote unreliable</span></div>` : mp.predHiBand ? `<div><span class="mp-label">Intraday high range</span><span class="mp-val">${rng(mp.predHiBand[0], mp.predHiBand[1])}</span></div>` : ''}
