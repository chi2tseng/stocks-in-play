...
<!-- Browser tab favicon — points to logo.png. Listed BEFORE the SVG fallback so browsers
     that support PNG icons (basically all modern browsers) pick the photographed S-mark.
     Older browsers without PNG support fall through to the inline-SVG below. -->
<link rel="icon" type="image/png" sizes="any" href="logo.png">
<!-- SVG fallback — kept as inline data URI so it ships with the HTML, no extra request.
     Cobalt rounded square + clean white S; rendered at small sizes if PNG fails to load. -->
...
<!-- Apple-touch + OG social-share preview use the photographed S-mark (high-res PNG). -->
